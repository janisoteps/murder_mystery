...
The single investigator is Mara Keller. The user approved the visual direction by generating `main_character/mara_keller_full_height_high_res_1.png`, then supplied the rigged model `main_character/mara_keller_3d_rigged_2.glb`. The spoiler-safe character playground now loads that model and applies only its thigh, shin, foot, and toe rotation tracks; the exported torso, head, arm, and hand animation tracks are excluded. A simple working flashlight is positioned at her hand and remains independent of the skeleton. The next work is iterative model improvement from the user's visual feedback; do not proceed to the map, NPC asset briefs, or location production until the investigator is approved.
